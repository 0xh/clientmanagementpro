	...
	$.widget('metroskin.formbuilder',{
		...
		_create: function(){
			...
			$('#build_code').on('click',function(){
				...
				t = that.transformAsp(t).replace(/&gt;/g,'>');
		...
		createText: function(form_layout){
			...
			container.append( '<div class="input-control text full-size"><input type="text" data-form-builder="text" id="TextBox" name="input_text" value="" /></div>' );
		...
		createTextarea: function(form_layout){
			...
			container.append( '<div class="input-control textarea full-size"><textarea data-form-builder="textarea" id="TextArea" name="input_textarea" ></textarea></div>' );
		...
		createCheckbox: function(form_layout){
			...
			container.find('label').append('<input type="checkbox" id="CheckBox" name="checkbox" data-form-builder="checkbox" value="" /><span class="check"></span><span class="caption"> Checkbox</span>');
		...
		createRadio: function(form_layout){
			...
			container.find('label').append('<input type="radio" id="RadioButton" name="radio" data-form-builder="radio" value="" /><span class="check"></span><span class="caption"> Radio</span>');
		...
		createSwitch: function(form_layout){
			...
			container.find('label').append('<input type="checkbox" id="Switch" name="switch" data-form-builder="switch" value="" /><span class="check"></span><span class="caption"> Switch</span>');
		...
		createSelect: function(form_layout){
			...
			container.append('<div class="input-control select full-size"><select id="DropDownList" name="select" data-form-builder="select">new SelectList(Enum.GetValues(typeof( @option ))),"Select One"</select></div>');
		...
		createFile: function(form_layout){
			...
			container.append('<div class="input-control file full-size" data-role="input"><input type="file" id="FileInput" name="file"/><button class="button" data-form-builder="file"><span class="mif-folder"></span></button></div>');
		...
		transformAsp: function(htm){

			htm = $(htm);

			$('input',htm).each(function(i,o){
				...
				switch(_type){
					case 'checkbox':
						_after = '@Html.CheckBoxFor( m => m.'+_id+' ) ';
					break;

					case 'radio':
						_after = '@Html.RadioButtonFor( m => m.'+_id+' ) ';
					break

					case 'hidden':
						_after = '@Html.HiddenFor( m => m.'+_id+' ) ';
					break;

					case 'password':
						_after = '@Html.PasswordFor( m => m.'+_id+' ) ';
					break

					case 'text':
					default:
						_after = '@Html.TextBoxFor( m => m.'+_id+' ) ';

					break;
				}

				tafter = $(this, htm ).after( _after );
				$(this,htm).remove();

			})

			$('textarea',htm).each(function(){
				var _after = '';
				var _id    = $(this).attr('id');

				_after = '@Html.TextAreaFor( m => m.'+_id+' )';

				tafter = $(this, htm ).after( _after );
				$(this,htm).remove();
			})

			$('select',htm).each(function(){
				var _after = '';
				var _id    = $(this).attr('id');
				var _opt   = $(this).html();

				_after = '@Html.DropDownListFor( m => m.'+_id+' , '+_opt+' )';
